...
public class OGLCanvas extends Canvas
{
  ...
  {
    // SystemOut.println ("OGLCanvas.paint");
    ...
    if (!hascontext)  // first draw into this Canvas
    {
      if (!loadedlib)
      {
        ...
        // SystemOut.println ("*** gejc lib needed now");
        SystemOut.println ("loading libgejc.so ...");
        try {
          System.loadLibrary ("gejc");
          loadedlib = true;
          // SystemOut.println ("*** gejc lib loaded");
        }
        catch (UnsatisfiedLinkError e)
        {
          SystemOut.print   ("OGLCanvas. [Error] unable to load 'gejc' shared library.");
          SystemOut.println ("  You'll get no display output. Please check your LD_LIBRARY_PATH.");
          // System.exit (1);
        }
      }
