 ...
 * 
 * Added missing ROUTE handling when read as a field
 * 28 Sep 15, Roy Walmsley
 */
...
abstract public class Node extends Object
{
  ...
  public final static String META_KEYWORD = "META";
  public final static String PROFILE_KEYWORD = "PROFILE";
  ...
  /** fields of the node */
  public Hashtable<String,Field> subfields = new Hashtable<String,Field> ();
...
//   }
  ...
  public Field getEvent (String name, int fclass)
  {
    ...
    {
      // SystemOut.println("found exact: " + name );
      return field;
    }
    ...
    boolean mkroute)
  {
    ...
    Field eventOut = fromNode.getEvent (fromEvent, Field.F_EVENTOUT);
    Field eventIn = toNode.getEvent (toEvent, Field.F_EVENTIN);
    Field exposedField = toNode.getEvent (toEvent, Field.F_EXPOSEDFIELD);

    if (eventOut == null || eventIn == null)
    {
//    if (eventOut == null && mkroute)           // eventOut not found
      if ((eventOut == null) && (exposedField == null) && mkroute)           // eventOut, exposedField not found
        pout.error ("[Node] [Error] ROUTE: Field (" + fromEvent + ") is not an eventOut or exposedField" + atCurrLine (st));

//    if (eventIn == null && mkroute)            // eventIn not found
      if ((eventIn == null) && (exposedField == null) && mkroute)            // eventIn, exposedField not found
        pout.error ("[Node] [Error] ROUTE: Field (" + toEvent + ") is not an eventIn or exposedField" + atCurrLine (st));
      return null;
    }
  ...
  public static Node readNode (VRMLparser parser)
  {
    ...
    // and destroy the whole scene graph
    String name;
    try
    {
    ...
    { pout.error ("[Node] [Error] IOException during parsing" + atCurrLine (st) + " " + e);
      return null;
    }

    // now we know the node name and unless it is one of the
    // DEF/USE/PROTO/ROUTE/PROFILE/META keywords we can call readNodeBody
...
//    SystemOut.println ("... reading " + whichproto + " " + protoname);
...
//    SystemOut.println ("... now ProtoNode node = new ProtoNode (protoname, external);");
...
//    SystemOut.println ("... now node.readProtoBody (parser);");
...
//    SystemOut.println ("... now parser.addProtoNode");

      parser.addProtoNode (protoname, node);  // register prototype

//    SystemOut.println ("... completed reading " + whichproto + " " + protoname);
    ...
    else if (name.equals ("META"))  // META
    {
    			pout.error ("META keyword not yet supported...");
    			/*
    			int quoteCount = 0;
    			while (quoteCount < 4)
    		  {
          	if (st.readChar().compareTo("\"") == 0) quoteCount++;
          // recurse, repeat
          readNode (parser);
          return null;
          }
    ...
    else if (name.equals ("PROFILE"))  // PROFILE
    {
    	pout.error ("PROFILE keyword not yet supported...");
    	String profileName = null;
    	
      try
      {
        profileName = st.readIdentifier ();
        SystemOut.println ("[Node] PROFILE name=" + profileName);
      }
      catch (IOException e)
      { pout.error ("[Node] [Error] IOException during parsing" + atCurrLine (st) + " " + e);
        return null;
      }
      // need to create ProfileNode here
      
      ProfileNode node = new ProfileNode (profileName);
      if (node == null)
        return null;
  ...
  static Node createInstanceFromName (VRMLparser parser, String name)
  {
    ...
    if (node != null)
    {
//    	SystemOut.println ("found node " + name + ", " +node);
      return node;
    }
    ...
    if (pnode != null)
    {
    	SystemOut.println ("found protonode " + name + ", " +pnode);
...
//  SystemOut.println("[Node] [Error] class '" + name + "' not found");
  ...
  void readNodeBody (VRMLparser parser, String name, char openbrace, char closebrace)
  {
    ...
    try
    {
      ...
      while (c != closebrace)
      {
        // SystemOut.println ("after readFields. got character: " + (char) c + " (" + c + ")");
        if (c == -1)
  ...
  void readFields (VRMLparser parser)
  {
    ...
    { pout.error ("[Node] [Error] IOException on reading fields " + atCurrLine (st) + " " + e);
      return;
    }
    ...
    while (!st.eof () && st.nextChar () != '}')  // ass.: comment was skipped before
    {
      ...
      if (fname.equals (ROUTE_KEYWORD))  // ROUTE
      {
        String fromNodeName = null, toNodeName = null;
        String fromEvent = null, toEvent = null;
        ...
        addRoute (fromNodeName, fromNode, fromEvent, toNodeName, toNode, toEvent, pout, st, true);
        continue;
      } // ROUTE

// TODO: [EXTERN]PROTOs are also allowed anywhere fields are

      Field field = null;
      if (fname.compareTo("collide") == 0)
        field = (Field) subfields.get ("enabled");
      else if (fname != null)
...
// 	  if (!field.readISdeclaration (parser,fieldIScontent)) {
	...
	/*
	  ...
	  */
      ...
      { pout.error ("[Node] [Error] IOException on or after reading field " + fname + atCurrLine (st) + " " + e);
        return;
      }
    } // until "}" or EOF
  ...
  public void writeNode (PrintStream os, Hashtable<String,Node> writtenrefs)
  {
  ...
  public void writeSubfields (PrintStream os, Hashtable<String,Node> writtenrefs)
  {
  ...
  public void writeSubfield (String fname, Field field, PrintStream os, Hashtable<String,Node> writtenrefs)
  {
  ...
  public void writeX3dNode (VRMLparser parser, PrintStream os, Hashtable<String,Node> writtenrefs, int depth, int proto) {
    if (proto == 1) {
   ...
   */
  public void writeX3dNode (VRMLparser parser, PrintStream os, Hashtable<String,Node> writtenrefs, int depth)
  {
   ...
   * write X3d node to output stream
   */
  public void writeX3dNode (PrintStream os, Hashtable<String,Node> writtenrefs, int depth)
  {
    if (nodeName().equals(ROUTE_KEYWORD)==true) {
      ((RouteNode) this).writeX3dNode(os, writtenrefs);
      return;
  ...
  public void writeX3dSubfields (PrintStream os, Hashtable<String,Node> writtenrefs,int depth)
  {
    // this works for all Nodes;
    // specific Nodes (longish ones, esp. geometry nodes) may prefer to
    // output their fields in the "canonical" order

    Vector<Field> saveNodes = new Vector<Field>();
    Vector<String> saveNames = new Vector<String>();
  ...
  public void writeX3dSubfields (VRMLparser parser, PrintStream os, Hashtable<String,Node> writtenrefs,int depth)
  {
    // this works for all Nodes;
    // specific Nodes (longish ones, esp. geometry nodes) may prefer to
    // output their fields in the "canonical" order
  ...
  public void writeX3dSubfield (String fname, Field field, PrintStream os, Hashtable<String,Node> writtenrefs)
  {
    // only fields (ordinary or exposed) written to output
