...
public class GroupNode extends Node implements GotEventCallback
{
  ...
  public void readNodes (VRMLparser parser, boolean stoponbrace)
  {
    ...
    Vector<Node> nodes = children.getNodes ();
...
//     	if (st.OutputComment().length() > 0) os.println (st.OutputComment());
        ...
        {
        	parser.pout.error ("node == null, ignored");
        	break;
      	}
      }
    }
    catch (IOException e)
    { parser.pout.error ("[GroupNode] [Error] IOException during parsing, line " + atCurrLine (st) + ", " + e);
  ...
  public void writeNodes (PrintStream os)
  {
    ...
    Hashtable<String,Node> writtenrefs = new Hashtable<String,Node> ();
