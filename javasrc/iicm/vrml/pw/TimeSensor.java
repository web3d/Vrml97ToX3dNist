...
public class TimeSensor extends Sensor
{
  ...
  {
    // SystemOut.println("TimeSensor: " + enabled.getValue ());
    if (enabled.getValue ())
    {
      if (!isActive.getValue ())
      {
        ...
        if (startTime_ == startTime.getValue () && !loop.getValue ())  // cycle for this start time already done
          return;
        // SystemOut.println(objname +" : Send isActive (true)");
        // SystemOut.println(objname +" : timestamp : " + double2string (timestamp));
        // SystemOut.println(objname +" : startTime : " + double2string (startTime.getValue ()));
        // SystemOut.println(objname +" : stopTime  : " + double2string (stopTime.getValue ()));
      ...
      // SystemOut.println(objname +" : Rest:" + f);
      ...
      if (newCycle_)
        ...
        {
          // SystemOut.println(objname +" : Send isActiv (false);  (timeout or loop == false)");
    ...
    {
      // SystemOut.println(objname +" : Send isActiv (false);  (enabeld == false)");
      isActive.setValue(false);
      isActive.sendEvent (timestamp);
      startTime.enableRoutes ();      // enable sending of starttime event
      stopTime.enableRoutes ();       // enable sending of stoptime event
