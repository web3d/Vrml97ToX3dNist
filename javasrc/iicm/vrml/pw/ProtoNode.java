...
public class ProtoNode extends GroupNode
{
  ...
  /** prototyped fields */
  public Hashtable<String,Field> protofields = new Hashtable<String,Field> ();
  ...
  void readProtoFields (VRMLparser parser)
  {
    ...
    try
    {
      while (st.skipCommentReturn (true) && !st.eof () && st.nextChar () != ']')
      {
        ...
        if (fclass == null ||
          !(fclass.equals (Field.STR_FIELD)
          || fclass.equals (Field.STR_EXPOSEDFIELD)
          || fclass.equals (Field.STR_EVENTIN)
          || fclass.equals (Field.STR_EVENTOUT)
          || fclass.equals (Field.STR_X3D_FIELD)
          || fclass.equals (Field.STR_X3D_EXPOSEDFIELD)
          || fclass.equals (Field.STR_X3D_EVENTIN)
          || fclass.equals (Field.STR_X3D_EVENTOUT)))
        {
          pout.error (nodeName () + " VRML97 fields must be declared as one of:\n" +
            Field.STR_FIELD + ", " + Field.STR_EXPOSEDFIELD + ", " +
            Field.STR_EVENTIN + ", " + Field.STR_EVENTOUT + ". Got " + fclass + atCurrLine (st));
          pout.error (nodeName () + " X3D ClassicVRML fields must be declared as one of:\n" +
            Field.STR_X3D_FIELD + ", " + Field.STR_X3D_EXPOSEDFIELD + ", " +
            Field.STR_X3D_EVENTIN + ", " + Field.STR_X3D_EVENTOUT + ". Got " + fclass + atCurrLine (st));
          return;
        }
        ...
        if (field != null)
        {
          ...
          if (fclass.equals (Field.STR_FIELD) || fclass.equals (Field.STR_X3D_FIELD) || fclass.equals (Field.STR_EXPOSEDFIELD) || fclass.equals (Field.STR_X3D_EXPOSEDFIELD))
          {
            if (fclass.equals (Field.STR_FIELD) || fclass.equals (Field.STR_X3D_FIELD))
          ...
          else if (fclass.equals (Field.STR_EVENTIN) || fclass.equals (Field.STR_X3D_EVENTIN))
            field.setFieldClass (Field.F_EVENTIN);
          else if (fclass.equals (Field.STR_EVENTOUT) || fclass.equals (Field.STR_X3D_EVENTOUT))
...
/*
  no conversion needed, url form is identical in X3D and VRML97
      int num = 0;
      if (urls_ != null) num = urls_.getValueCount();
      for (int i=0; i<num; i++) {
