...
public class Script extends Common
{
  ...
  void readFields (VRMLparser parser)
  {
    ...
    try
    {
      while (st.skipCommentReturn (true) && !st.eof () && st.nextChar () != '}')
      {
        ...
        {
          pout.error ("VRML97 Script fields must be declared as one of:\n" +
            Field.STR_FIELD + ", "  + Field.STR_EVENTIN + ", " + Field.STR_EVENTOUT +
            ". Got " + fclass + atCurrLine (st));
          pout.error (nodeName () + " X3D ClassicVRML Script fields must be declared as one of:\n" +
	  ...
	  if (if_is) {
		...
		pout.error ("Script field " + fidname + " had invalid data" + atCurrLine (st));
          ...
          if (fclass.equals (Field.STR_X3D_EXPOSEDFIELD))  // not allowed in VRML97
          {
            addField (fidname, field, Field.F_EXPOSEDFIELD);
            // field.setFieldClass (Field.F_FIELD);
            // read default field value
	    if (if_is != true) {
	      field.readValue (parser);
  ...
  @Override
  public void writeSubfield (String fname, Field f, PrintStream os, Hashtable<String,Node> writtenrefs)
  {
    // ordinary field
